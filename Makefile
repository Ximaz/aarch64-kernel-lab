...
dtb_tree_vizualizer: dtb_tree_vizualizer.c
	$(GCC) $< -o $@
...
fclean: clean
	rm -f $(KERNEL_IMG) $(KERNEL_ELF) bootloader_communication dtb_tree_vizualizer

re: fclean all

.PHONY: all asm elf_headers debug release bootloader_communication dtb_tree_vizualizer clean fclean re
